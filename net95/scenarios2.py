...
def get_data_by_scenario_and_case(scenario, case, net_name='net95v1'):
    assert case > 0
    assert scenario > 0
    ...
    x = np.hstack((
        [list(np.asarray(valori_veri_misure[4:4+94]))], [list(np.asarray(valori_veri_misure[4+94:4+94+94]))], [valori_veri_misure[:4]], [valori_veri_misure[192:199]], [valori_veri_misure[199:]]
    ))

    x_hat = np.hstack((
        [list(np.asarray(misure[4:4+94]))], [list(np.asarray(misure[4+94:4+94+94]))], [misure[:4]], [misure[192:199]], [misure[199:]]
    ))
    y = [valori_veri_stima[:95]]
    y_hat = [valori_stimati[:95]]
